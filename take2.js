...
function genRule (ruleFn) {
    ...
    function * runRule (db, bindings, q, rows) {
        const [row, ...restBody] = rows
        for (const rule of searchSpace(db, row)) {
            const nextBindings = unify(bindings, row, rule.index)
            if (!nextBindings) { continue }
            if (rows === body) console.log('')

            if (restBody.length) {
                console.log('-', { row, rule: rule.index })
                yield * runRule(db, nextBindings, q, restBody)
            } else {
                console.log('++', { row, rule: rule.index })
                if (unify(nextBindings, q, head)) {
                    console.log('>>>', {q, head})
                }
                yield nextBindings
            }
